function [idx,xi,idx2,slope] = get_boundary(fn,bplot)
...
Dof = inline('(beta(1)*exp(-beta(1)*(x-beta(2))))./((1+exp(-beta(1)*(x-beta(2)))).^2)','beta','x');

beta1 = nlinfit([1:N]',0.1*nansum(res1,2),of,[1,N/2]);
beta2 = nlinfit([1:N]',0.1*nansum(res2,2),of,[-1,N/2]);
%%%
beta11 = nlinfit(linspace(start_idx,end_idx,N)',0.1*nansum(res1,2),of,[1,start_idx+(end_idx-start_idx)/2]);
beta21 = nlinfit(linspace(start_idx,end_idx,N)',0.1*nansum(res2,2),of,[-1,start_idx+(end_idx-start_idx)/2]);

% [xi,yi] = polyxpoly(1:N,nansum(res1,2),1:N,nansum(res2,2),'unique');
[xi,yi] = polyxpoly(1:N,of(beta1,1:N),1:N,of(beta2,1:N),'unique');

slope(1) = beta11(1); %Dof(beta1,xi);
slope(2) = beta21(1); %Dof(beta2,xi);

idx = max(start_idx,min(end_idx,start_idx+round((xi-1)/(N-1)*(end_idx-start_idx))));
idx2 = fileidx(round(xi));

% slope(1) = Dof(beta11,idx);
% slope(2) = Dof(beta21,idx);
